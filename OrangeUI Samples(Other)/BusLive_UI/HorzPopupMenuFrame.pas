...
type
  ...
  TFrameHorzPopupMenu = class(TFrame)
...
var
  GlobalPopupMenuFrame:TFrameHorzPopupMenu;
...
begin
  ...
  begin
    GlobalPopupMenuFrame:=TFrameHorzPopupMenu.Create(Application);
  end;
...
procedure TFrameHorzPopupMenu.BackRectangleClick(Sender: TObject);
begin
  HidePopupMenu;
end;

constructor TFrameHorzPopupMenu.Create(AOwner: TComponent);
begin
  inherited;
  Self.OnResize:=Self.FrameResize;
end;

destructor TFrameHorzPopupMenu.Destroy;
begin
  GlobalPopupMenuFrame:=nil;
  inherited;
end;

procedure TFrameHorzPopupMenu.DoParentResize(Sender: TObject);
begin
  Self.Left:=0;
  Self.Top:=0;
...
procedure TFrameHorzPopupMenu.FrameResize(Sender: TObject);
var
  AWidth:Integer;
begin
  AWidth:=0;
...
procedure TFrameHorzPopupMenu.HidePopupMenu;
begin
...
procedure TFrameHorzPopupMenu.lblCaptionClick(Sender: TObject);
begin
  HidePopupMenu;
end;

procedure TFrameHorzPopupMenu.lbMenusClickItem(Sender: TObject);
begin
...
procedure TFrameHorzPopupMenu.ShowPopupMenu(AParent:TFmxObject;
                                        const ACaption:String;
                                        AMenus:Array of String;
                                        AMenuEvent:TMenuClickEvent;
                                        ATag:Integer;
...
procedure TFrameHorzPopupMenu.tmrBringToFontTimer(Sender: TObject);
begin
  Self.BringToFront;
  //在对话框的父控件中对齐对话框
  Self.DoParentResize(Parent);
