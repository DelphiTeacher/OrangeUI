...
type
  ...
  private
    ...
    //wn//如果是整型,会报错,所以加了这个方法
    function  GetDoubleValue(const Name: Typ): IJSONAncestor;
...
begin
  ...
  begin
    //wn
    AJSONPair:=TJSONObject(FInterface).Get(TValue.From<Typ>(Name).AsString);
    if AJSONPair.JsonValue is TJSONNull then
      Result := Nil
    else
    begin
//      if Result is TJSONFloat then
//      begin
//          //{"money": 8}
//          if AJSONPair.JSonValue is TJSONInteger then
//          begin
//            //如果是整型,会报错
//
//          end
//          else
//          begin
//            Result := AJSONPair.JSonValue as C;
//          end;
//      end
//      else
//      begin
...
function TBaseJSON<T, Typ>.GetDoubleValue(const Name: Typ): IJSONAncestor;
var
  AJSONPair:IJSONPair;
begin
  if Self.InheritsFrom(TSuperObject) then
  ...
  if Self.InheritsFrom(TSuperArray) then
    Result := TJSONArray(FInterface).Get(TValue.From<Typ>(Name).AsInteger)// as IJSONAncestor
  else
    Result := Nil;
end;
...
begin
  ...
  begin
    //wn
    Result := GetDoubleValue(V).AsVariant;
    //.ValueEx<Double>;
  end;
end;
...
begin
  Result := XSuperObject_Copy.TSerializeParse.ReadRecordEx<T>(Rec);
end;
...
begin
  ...
  if pV = TypeInfo(Double) then begin
     if Valuable then
     begin
        if Self is TJSONFloat then
        begin
          Result := (Self as TJSONFloat).Value;
        end
        else if Self is TJSONInteger then
        begin
          Result := (Self as TJSONInteger).Value;
        end
        else
