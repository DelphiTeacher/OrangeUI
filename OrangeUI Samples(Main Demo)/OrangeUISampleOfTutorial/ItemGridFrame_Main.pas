...
type
  TFrameItemGrid_Main = class(TFrame,IFrameChangeLanguageEvent)
...
procedure TFrameItemGrid_Main.ChangeLanguage(ALangKind: TLangKind);
begin
//  //翻译
...
//  Self.lbDemos.Prop.Items.FindItemByCaptionContains('(Test Grid)').Caption:=GetLangString('(Test Grid)',ALangKind);
//
...
constructor TFrameItemGrid_Main.Create(AOwner: TComponent);
begin
  inherited;
...
//  RegLangString('(Test Grid)',
//    [Self.lbDemos.Prop.Items.FindItemByCaptionContains('(Test Grid)').Caption,'(Test Grid)']);
//
//
...
procedure TFrameItemGrid_Main.lbDemosClickItem(Sender: TSkinItem);
begin
...
//  TFrameDemoView(FDemoViewFrame).FrameHistroy:=CurrentFrameHistroy;
  TFrameDemoView(FDemoViewFrame).ShowDemo(Sender.Caption,'ItemGrid');
