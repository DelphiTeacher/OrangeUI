...
unit ViewMode_CenterSelect_SelectCityFrame;
...
uses
  ...
  uSkinCustomListType, uSkinFireMonkeyCustomList, uSkinPanelType, uDrawCanvas;

const
  {$region '省份城市常量'}
  CONST_PROVINCES: string =
  '安徽省,北京,福建省,甘肃省,广东省,广西壮族自治区,贵州省,海南省,河北省,河南省,' +
  '黑龙江省,湖北省,湖南省,吉林省,江苏省,江西省,辽宁省,内蒙古自治区,宁夏回族自治区,' +
  '青海省,山东省,山西省,陕西省,上海,四川省,天津,西藏自治区,新疆维吾尔自治区,' +
  '云南省,浙江省,重庆,香港,澳门,台湾';
...
type
  TFrameViewMode_CenterSelect_SelectCity = class(TFrame)
...
procedure TFrameViewMode_CenterSelect_SelectCity.btnCancelClick(Sender: TObject);
begin
...
procedure TFrameViewMode_CenterSelect_SelectCity.btnOKClick(Sender: TObject);
begin
  HideFrame;////(Self,hfcttBeforeReturnFrame);
  ReturnFrame;//(FrameHistroy);
end;

procedure TFrameViewMode_CenterSelect_SelectCity.btnReturnClick(Sender: TObject);
begin
  if GetFrameHistory(Self)<>nil then GetFrameHistory(Self).OnReturnFrame:=nil;
...
constructor TFrameViewMode_CenterSelect_SelectCity.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TFrameViewMode_CenterSelect_SelectCity.FrameResize(Sender: TObject);
begin
  Self.lbProvince.Width:=Self.Width/2;
  Self.lbCity.Width:=Self.Width/2;
end;

procedure TFrameViewMode_CenterSelect_SelectCity.lbProvinceSelectedItem(Sender: TObject;AItem:TSkinItem);
begin
  //加载城市
  LoadCommaText(lbCity,CONST_CITYS[AItem.Index]);
end;

procedure TFrameViewMode_CenterSelect_SelectCity.LoadCommaText(AListBox: TSkinFMXListBox;ACommaText: String);
var
  I: Integer;
...
begin
  ...
  try
    ...
    uFuncCommon.FreeAndNil(AStringList);
  end;
end;
