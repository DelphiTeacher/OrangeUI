...
{ TClientDataSet }
  ...
  public type
    // published TPersistent property allows control over order of published property propogation to descendent views or forms.
    // The datapacket must be propogated before the Active property, so the PersistDataPacket
    // property is declared before the Active property.
    TPersistDataPacket = class(TPersistent)
    private
    ...
    public
      constructor Create(const ADataSet: TCustomClientDataSet);
    end;
  private
    FPersistDataPacket: TPersistDataPacket;
...
{$ENDIF !NEXTGEN}
    ...
    procedure SetPersistDataPacket(const Value: TPersistDataPacket);
    function DataStored(const AAncestor: TCustomClientDataSet): Boolean;
  protected
    { IProviderSupport }
...
{$ENDIF !NEXTGEN}
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property PersistDataPacket: TPersistDataPacket read FPersistDataPacket write SeTPersistDataPacket;
    property Active;
...
begin
  inherited Create(AOwner);
  FPersistDataPacket := TPersistDataPacket.Create(Self);
  FRanged := False;
...
procedure TCustomClientDataSet.SetPersistDataPacket(
  const Value: TPersistDataPacket);
begin
  // Do not delete this mehod.
  // PersistDataPacket property must be declared read/write.
end;
...
function TCustomClientDataSet.ParamByName(const Value: string): TParam;
begin
...
function TCustomClientDataSet.DataStored(const AAncestor: TCustomClientDataSet): Boolean;
begin
  Result := Active and (DataSetField = nil) and not HasAppServer and
    (ProviderName = '') and (FCloneSource = nil);
  if Result and Assigned(AAncestor) then
    Result := not TCustomClientDataSet(AAncestor).Active or TCustomClientDataSet(AAncestor).HasAppServer or (TCustomClientDataSet(AAncestor).ProviderName <> '') or
      (TCustomClientDataSet(AAncestor).DataSize <> Self.DataSize);
end;

procedure TCustomClientDataSet.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  // Register Data property reader to load from old forms, new reader and writer is implement by TPersistDataPacket
  ...
  for I := 0 to FAggregates.Count - 1 do
    begin
      if (FAggregates[I].AggHandle <> 0) and (FDSCursor <> nil) then
        FDSCursor.DropAggregate(FAggregates[I].AggHandle);
...
{ TCustomClientDataSet.TDataPersistent }

constructor TCustomClientDataSet.TPersistDataPacket.Create(
  const ADataSet: TCustomClientDataSet);
begin
  inherited Create;
  FDataSet := ADataSet;
end;

procedure TCustomClientDataSet.TPersistDataPacket.DefineProperties(Filer: TFiler);
var
  LAncestor: TCustomClientDataSet;
begin
  inherited;
  if Filer.Ancestor is TPersistDataPacket then
    LAncestor := TPersistDataPacket(Filer.Ancestor).FDataSet
  else
    LAncestor := nil;
  Filer.DefineBinaryProperty('Data', FDataSet.ReadData, FDataSet.WriteData, FDataSet.DataStored(LAncestor));
end;
