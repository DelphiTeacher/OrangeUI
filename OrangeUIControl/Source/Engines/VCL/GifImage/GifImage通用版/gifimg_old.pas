...
{$ENDIF}
  ...
  DoAutoDither: boolean = True;//False;
  ...
  for i := 0 to FHeader.Image.Images.Count-1 do
    ...
    begin
      ...
      *)
      while (Pixel < LastPixel) do
      begin
        Pixel^ := ansichar(chr(Map[ord(Pixel^)]));
        inc(Pixel);
      end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        if (x AND $01 = 0) then
          Dest^ := ansichar(chr(ord(ScanLine^) SHR 4))
        else
        begin
          Dest^ := ansichar(chr(ord(ScanLine^) AND $0F));
          inc(ScanLine);
        end;
        inc(Dest);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Dest^ := ansichar(chr((Byte AND $80) SHR 7));
        Byte := Byte SHL 1;
        inc(Dest);
        dec(Bit);
        dec(x);
    ...
    begin
      ...
      begin
        ...
        repeat
          ...
          begin
            Dest^ := ansichar(chr(Cache[i].Index));
            LastEntry := i;
            goto NextPixel;
          end;
          if (i = 0) then
        ...
        Dest^ := ansichar(chr(FColorMap.AddUnique(Pixel)));
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if (MaskPixel^ <> #0) then
          begin
            GIFPixel^ := ansichar(chr(TransparentIndex));
            WasTransparent := True;
          end;
          inc(MaskPixel);
          inc(GIFPixel);
