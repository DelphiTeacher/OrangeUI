...
//little modify for support fmx by delphiteacher !  2014-04-13 13:52 Sun.
unit uSkinGIFImage;
...
// {$IFDEF VER280}TSeekOrigin.soFromCurrent{$ENDIF}{$IFDEF VER290}TSeekOrigin.soCurrent{$ENDIF}
...
type
  ...
  GIFException = class(Exception);//(EInvalidGraphic);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public
    ...
    property Image: TSkinGIFImage read FGIFImage;
  end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public
    ...
    property Image: TSkinGIFImage read FImage;
  end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public
    constructor Create(GIFImage: TSkinGIFImage); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure SaveToStream(Stream: TStream); override;
    procedure LoadFromStream(Stream: TStream); override;
    class function Check(Stream: TStream):Boolean;
    procedure Clear;
    property Version: TGIFVersion read GetVersion;
...
//    FLocalPalette	: HPalette;
    ...
    function GetSkinPicture: TObject;
  protected
    function GetVersion: TGIFVersion; override;
...
//    procedure SetPalette(Value: HPalette);
    ...
    procedure FreeImage;
    procedure NeedImage;
    function ScaleRect(DestRect: TRectF): TRectF;
...
//    procedure Merge(Previous: TGIFSubImage);
    ...
    property SkinPicture:TObject read GetSkinPicture;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public
    constructor Create(ASubImage: TGIFSubImage); override;
    destructor Destroy; override;
    property Blocks: TBaseList read FBlocks;
  end;
...
//     goAutoDither		// Only dither on 256 color systems
  ...
  private
    FPaintObject:TObject;
    Disposals		: set of TDisposalMethod;
    FImage		: TSkinGIFImage;	// The TSkinGIFImage that owns this painter
//    FCanvasControl		: TCanvas;	// Destination canvas
...
//    procedure Execute;// override;
  ...
  public
    constructor Create(AImage: TSkinGIFImage; //ACanvas: TCanvas;
                          //ARect: TRect;
                          PaintObject:TObject;
                          Options: TGIFDrawOptions);
                    //    constructor CreateRef(Painter: PGIFPainter;
                    //      AImage: TSkinGIFImage;
                    //      //ACanvas: TCanvas;
                    //      //ARect: TRect;
                    //      PaintObject:TObject;
                    //      Options: TGIFDrawOptions);
...
//    Buffer        : TBitmap;
    ...
    BackupBufferCanvas:TObject;
    FrameBuffer		: TBitmap;	// Used by synchronized paint
    FrameBufferCanvas:TObject;
//    Background		: TBitmap;	// Used by synchronized paint
    ...
    procedure DoPaintFrame(FCanvas:TObject;ADrawPictureParam:TDrawPictureParam;FRect:TRectF);		// Sync. buffered paint procedure
    procedure DoPaint(FCanvas:TObject;ADrawPictureParam:TDrawPictureParam;FRect:TRectF);			// Sync. paint procedure


//    procedure Paint(FCanvas:TCanvas;FRect:TRect);			// Sync. paint procedure

    property Image: TSkinGIFImage read FImage;
    property PaintObject:TObject read FPaintObject;
...
//    property EventHandle: THandle read FEventHandle;
  ...
  TGIFPainterList=class(TBaseList)
  private
    function GetItem(Index: Integer): TGIFPainter;
  public
    function FindByPaintObject(PaintObject:TObject):TGIFPainter;
    property Items[Index:Integer]:TGIFPainter read GetItem;Default;
  end;
  ...
  TSkinGIFImage = class(TPersistent)//(TGraphic) // (TComponent)//
  private
//    IsDrawing		: Boolean;
...
//
//    FGIFFile:TMemoryStream;
//    function GetGIFFile: TMemoryStream;
//    procedure SetGIFFile(const Value: TMemoryStream);
//    procedure SetGIFFileStream(const Value: TStream);
//{$IFDEF VER9x}
...
//    procedure WriteData(Stream: TStream); //override;
    ...
    procedure SetAnimationSpeed(Value: integer);
    procedure SetGIFDelayExp(Value: integer);
...
//    procedure StopDraw;
    ...
    function Paint(PaintObject:TObject;
                  Options: TGIFDrawOptions): TGIFPainter;
//    function InternalPaint(Painter: PGIFPainter;
//                PaintObject:TObject;
//                //ACanvas: TCanvas; const Rect: TRect;
//                Options: TGIFDrawOptions): TGIFPainter;
//    procedure Draw(PaintObject:TObject);//ACanvas: TCanvas; const Rect: TRect); //override;
...
//    property DitherMode: TDitherMode read FDitherMode write FDitherMode;
    ...
    property Painters: TGIFPainterList read FPainters;
...
//    property GIFFile:TMemoryStream read GetGIFFile write SetGIFFile;
...
//  TSkinGIFImage=class(TComponent)
//  private
//    FGIFImage:TSkinGIFImage;
//    FOnPaint: TNotifyEvent;
//    procedure SetGIFFile(const Value: TMemoryStream);
...
//    procedure LoadFromStream(Stream: TStream); //override;
//  published
//    property GIFImage:TSkinGIFImage read FGIFImage;
//    property GIFFile:TMemoryStream read FGIFFile write SetGIFFile;
//    property OnPaint:TNotifyEvent read FOnPaint write FOnPaint;
//  end;
...
////////////////////////////////////////////////////////////////////////////////
var
  //: Default options for TSkinGIFImage.DrawOptions.
...
//  GIFImageDefaultThreadPriority: TThreadPriority = tpNormal;
  ...
  GIFImageDefaultAnimationSpeed: integer = 10;
  ...
  // This will not affect TSkinGIFImage.SaveToStream or SaveToFile.
...
type
  byte7array=Array [0..7] of Byte;

var
  IsClearDrawDestinationCanvas:Boolean;
...
procedure CopyStringToByte7Array(Str:String;byte7array:TGIFIdentifierCode);
var
  I: Integer;
begin
  for I := 0 to Length(byte7array)-1 do
  begin
    byte7array[I]:=Ord(Str[I+1]);
  end;
end;

function Byte7ArrayToStr(byte7array:TGIFIdentifierCode):String;
var
  I: Integer;
begin
  for I := 0 to Length(byte7array)-1 do
  begin
    Result:=Result+' ';
    Result[I+1]:=Char(byte7array[I]);
  end;
end;

procedure CopyStringToByte2Array(Str:String;byte2array:TGIFAuthenticationCode);
var
  I: Integer;
begin
  for I := 0 to Length(byte2array)-1 do
  begin
    byte2array[I]:=Ord(Str[I+1]);
  end;
end;

function Byte2ArrayToStr(byte2array:TGIFAuthenticationCode):String;
var
  I: Integer;
begin
  for I := 0 to Length(byte2array)-1 do
...
// Since TSkinGIFImage isn't a component there's nothing to register here, but
...
**  of max 255 ansicharacters in each.
*)
...
type
  ...
  private
    FColors		: integer;
  public
    constructor Create;virtual;//(Palette: hPalette); virtual;
...
//  public
//    constructor Create;override;//(Palette: hPalette); override;
//    destructor Destroy; override;
...
//begin
////  Result := Byte(GetNearestPaletteIndex(FPalette, Red OR (Green SHL 8) OR (Blue SHL 16)));
//  if (FPaletteEntries <> nil) then
//    with FPaletteEntries^[ord(Result)] do
//    begin
//      R := peRed;
...
begin
  Result := Byte((Blue*29 + Green*150 + Red*77) DIV 256);
  R := ord(Result);
  G := ord(Result);
  B := ord(Result);
end;
...
//    FLookup		: TColorLookup;
    Width		: integer;
  public
    constructor Create(AWidth: integer//; Lookup: TColorLookup
    ); virtual;
...
////////////////////////////////////////////////////////////////////////////////
//	TDitherEngine
constructor TDitherEngine.Create(AWidth: integer
//; Lookup: TColorLookup
);
begin
  inherited Create;
...
//
////////////////////////////////////////////////////////////////////////////////
constructor TGIFItem.Create(GIFImage: TSkinGIFImage);
begin
  inherited Create;
...
//
////////////////////////////////////////////////////////////////////////////////
constructor TGIFList.Create(Image: TSkinGIFImage);
begin
  inherited Create;
  FImage := Image;
//  FItems := TList.Create;
...
//
////////////////////////////////////////////////////////////////////////////////
constructor TGIFHeader.Create(GIFImage: TSkinGIFImage);
begin
...
//  GifHeader.Signature := 'GIF';
  GifHeader.Signature[0] := Ord('G');
  GifHeader.Signature[1] := Ord('I');
  GifHeader.Signature[2] := Ord('F');
...
//  GifHeader.Version := GIFVersions[v];
  case v of
    gv87a:
    begin
//      GifHeader.Version:='87a';
      GifHeader.Version[0] := Ord('8');
      GifHeader.Version[1] := Ord('7');
      GifHeader.Version[2] := Ord('a');
    end;
    gv89a:
    begin
//      GifHeader.Version:='89a';
      GifHeader.Version[0] := Ord('8');
      GifHeader.Version[1] := Ord('9');
      GifHeader.Version[2] := Ord('a');
    end;
  end;
...
class function TGIFHeader.Check(Stream: TStream): Boolean;
var
  GifHeader		: TGIFHeaderRec;
//  ColorCount		: integer;
  Position		: integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while (b <> 0) do
        begin
          Stream.Seek(b, TSeekOrigin.soCurrent);
          if (Stream.Read(b, 1) <> 1) then
            exit;
        end;
        exit;
...
type
  ...
  // - The lower 8 bits are the postfix ansicharacter (the new pixel).
...
begin
  ASSERT(longInt($FFFFFFFF) = -1, 'TSkinGIFImage implementation assumes $FFFFFFFF = -1');
...
constructor TGIFSubImage.Create(GIFImage: TSkinGIFImage);
begin
  inherited Create(GIFImage);
...
begin
  if (FGIFImage <> nil) then
    FGIFImage.Images.Remove(self,False);
  Clear;
...
//  DIBResult		: TDIB;
  ...
  Row			: integer;
  Color			: TGIFColor;
//  AlphaColor:: TGIFColor;
  ColMap		: PColorMap;
  Index			: byte;
  TransparentIndex	: byte;
  IsTransparent		: boolean;
//  WasTransparent	: boolean;
//  MappedTransparentIndex: Byte;
...
//      DIBResult := TDIBWriter.Create(Result, pf8bit, Width, Height, Palette);
      {$IFDEF FMX}
      Result.Map(TMapAccess.{$IF CompilerVersion >= 35.0}Write{$ELSE}maWrite{$IFEND},Data);
      {$ENDIF}

      // Determine if this image is transparent
      ColMap := ActiveColorMap.Data;
      IsTransparent := //FNeedMask and
                        Transparent;
...
//        MappedTransparentIndex := Byte(Color.Blue DIV 51 +
...
//          Result.Map(TMapAccess.maReadWrite,Data);


          {$IFDEF FMX}
          Dst := Data.GetScanLine(Row);
          {$ENDIF}
          {$IFDEF VCL}
          Dst := Result.ScanLine[Row];
          {$ENDIF}

          if (IsTransparent) then
          begin
            // Preset all pixels to transparent
            FillChar(Dst^, Width*4, #0);//ord(MappedTransparentIndex));
          ...
          if Ditherer.Direction>0 then
          begin
            inc(Dst, Ditherer.Column*4);
          end
          else
          begin
            inc(Dst, (Ditherer.Column+1)*4-1);
          end;
          ...
          begin
            ...
            begin
              MaskDest^ := Byte(byte(MaskDest^) OR Bit);
//              WasTransparent := True;
              Ditherer.NextColumn;
              Alpha:=0;
              Inc(Dst,Ditherer.Direction);
            ...
            begin
              //222,181,164
              // Dither and map a single pixel
//              Ditherer.Dither(Color.Red, Color.Green, Color.Blue,
...
//              {XE6}
//              {$IFDEF VER260}
//              if Result.PixelFormat=pfA8R8G8B8 then
//              {$ENDIF}
//              {$IFDEF VER270}
//              if Result.PixelFormat=TPixelFormat.BGRA then
//              {$ENDIF}
//              {$IFDEF VER280}
              if Result.PixelFormat=TPixelFormat.BGRA then
//              {$ENDIF}
              ...
              {XE6}
//              {$IFDEF VER260}
//              else if Result.PixelFormat=pfA8B8G8R8 then
//              {$ENDIF}
//              {$IFDEF VER270}
//              else if Result.PixelFormat=TPixelFormat.RGBA then
//              {$ENDIF}
//              {$IFDEF VER280}
              else if Result.PixelFormat=TPixelFormat.RGBA then
//              {$ENDIF}
...
//        begin
//          Dst := Data.GetScanline(ScanLineRow);
//          //DIBResult.ScanLine[ScanLineRow];
//
...
//              if (Src^ = Byte(TransparentIndex)) then
...
begin
  ...
  Result := BYTE(PByte(longInt(Scanline[y]) + x)^);
end;
...
function TGIFSubImage.GetSkinPicture: TObject;
begin
  Self.GetBitmap;
  if FSkinPicture=nil then
  begin
    FSkinPicture:=uDrawEngine.CreateCurrentEngineSkinPicture;
    TSkinPicture(FSkinPicture).Assign(FBitmap);
  end;
...
function TGIFSubImage.ScaleRect(DestRect: TRectF): TRectF;
var
  HeightMul		,
  HeightDiv		: integer;
  WidthMul		,
...
begin
  ...
  HeightMul := Ceil(DestRect.Bottom-DestRect.Top);
  WidthDiv := Image.Width;
  WidthMul := Ceil(DestRect.Right-DestRect.Left);
...
//  begin
//    pSource := PByte(integer(Previous.Scanline[PreviousY]) + MergeRect.Left - Previous.Left);
//    pDest := PByte(integer(Scanline[Y]) + MergeRect.Left - Left);
//
...
//      if (
//            not(
//              ((not NeedTransparentColorIndex) and (pDest^ = Byte(DestIndex))) or
//              ((SourceTransparent) and (pSource^ = Byte(SourceIndex)))
//            )
//          ) and (
...
begin
  if (FSubImage <> nil) then
    FSubImage.Extensions.Remove(self,False);
  inherited Destroy;
end;
...
begin
  ...
  CopyStringToByte7Array('NETSCAPE',NSLoopIdent[0].Identifier);
  CopyStringToByte2Array('2.0',NSLoopIdent[0].Authentication);

  CopyStringToByte7Array('ANIMEXTS',NSLoopIdent[1].Identifier);
  CopyStringToByte2Array('1.0',NSLoopIdent[1].Authentication);
...
begin
  Result := Byte2ArrayToStr(FIdent.Authentication);
end;
...
//  StrLCopy(@(FIdent.Authentication[0]), PByte(Value), sizeof(TGIFAuthenticationCode));
end;

function TGIFApplicationExtension.GetIdentifier: string;
begin
  Result := Byte7ArrayToStr(FIdent.Identifier);
end;
...
//  StrLCopy(@(FIdent.Identifier[0]), PByte(Value), sizeof(TGIFIdentifierCode));
end;
...
begin
  ...
  Dec(i, sizeof(FIdent));
  // Ignore extra data
  Stream.Seek(i, TSeekOrigin.soCurrent);
...
//  begin
//    AGIFBlock:=TGIFBlock(FBlocks[i]);
//    FreeAndNil(AGIFBlock);
//  end;
  FreeAndNil(FBlocks);
  inherited Destroy;
end;
...
begin
  ...
  CopyStringToByte7Array('NETSCAPE',NSLoopIdent.Identifier);
  CopyStringToByte2Array('2.0',NSLoopIdent.Authentication);
...
begin
  ...
  begin
    ...
    begin
      SubImage := TGIFSubImage.Create(Parent as TSkinGIFImage);
      try
...
//
//////////////////////////////////////////////////////////////////////////////////
//constructor TGIFPainter.CreateRef(Painter: PGIFPainter;
//                                  AImage: TSkinGIFImage;
//                                  //ACanvas: TCanvas;
//                                  //ARect: TRect;
//                                  PaintObject:TObject;
...
constructor TGIFPainter.Create(AImage: TSkinGIFImage; //ACanvas: TCanvas; ARect: TRect;
  PaintObject:TObject;
  Options: TGIFDrawOptions);
var
  i			: integer;
...
//  FEventHandle := 0;
  ...
  GIFDelayExp := FImage.GIFDelayExp;

  //var
  {$IFDEF FAST_AS_HELL}
    GIFDelayExp:= 10;		// Delay multiplier in mS.
  {$ELSE}
    GIFDelayExp:= 12;		// Delay multiplier in mS. Tweaked.
  {$ENDIF}
  // An event handle is used for animation delays
  if (FDrawOptions >= [goAnimate//, goAsync
      ]) and (FImage.Images.Count > 1) and
    (FAnimationSpeed >= 0) then
  begin
    FEventTimer:=TTimer.Create(nil);
    FEventTimer.OnTimer:=DoEventTimer;
...
//  if (dmBackground in Disposals) then//and (goTransparent in FDrawOptions) then
//  begin
...
//    Background.Canvas.CopyBitmap(Background.Canvas.ClipRect, FCanvas.Bitmap, FRect);
//  end;
...
//   Need frame- and backup buffer to restore to previous and background
  ...
  begin
    ...
    BackupBuffer.Height := AImage.Height;//FRect.Bottom-FRect.Top;
    BackupBuffer.Width := AImage.Width;//FRect.Right-FRect.Left;
    FreeAndNil(BackupBufferCanvas);
    BackupBufferCanvas:=uDrawEngine.CreateDrawCanvas('TGIFPainter');
    TDrawCanvas(BackupBufferCanvas).Prepare(BackupBuffer.Canvas);
//    BackupBuffer.Canvas.Fill.Kind:=FMX.Graphics.TBrushKind.bkSolid;
//    BackupBuffer.Canvas.Fill.Color:=BackgroundColor;
//    BackupBuffer.Canvas.CopyMode := cmSrcCopy;
//    BackupBuffer.Canvas.Color := BackgroundColor;
...
//    BackupBuffer.Canvas.CopyRect(BackupBuffer.Canvas.ClipRect, FCanvas, FRect);
    ...
    FrameBuffer.Height := AImage.Height;//.Bottom-FRect.Top;
    FrameBuffer.Width := AImage.Width;//.Right-FRect.Left;
    FrameBufferCanvas:=uDrawEngine.CreateDrawCanvas('TGIFPainter');
    TDrawCanvas(FrameBufferCanvas).Prepare(FrameBuffer.Canvas);
//    FrameBuffer.Canvas.Fill.Kind:=FMX.Graphics.TBrushKind.bkSolid;
//    FrameBuffer.Canvas.Fill.Color:=BackgroundColor;
...
//      SetEvent(FEventHandle)
    if Self.FEventTimer<>nil then
    begin
      DoRestart := True;
      Self.FEventTimer.Interval:=FAnimationSpeed*GIFDelayExp;
      Self.FEventTimer.Enabled:=True;
    end;
//    else
//      DoRestart := True;
  end;
...
procedure TGIFPainter.FireEvent(Event: TNotifyEvent);
begin
...
//procedure TGIFPainter.Paint(FCanvas: TCanvas; FRect: TRect);
//begin
//  // Paint the image
//  if (BackupBuffer <> nil) then
//  begin
//    DoPaintFrame(FCanvas,FRect);
//  end
//  else
//  begin
//    DoPaint(FCanvas,FRect);
//  end;
//end;

procedure TGIFPainter.DoEventTimer(Sender:TObject);
var
  i			: integer;
  Ext			: TGIFExtension;
//  Msg			: TMsg;
...
//    try
        ...
        // Loop to loop - duh!
        if ((LoopCount <> 0) or (goLoopContinously in DrawOptions)) and not(//Terminated or
                                                                            DoRestart) then
        begin
          if CanInc then
          begin
...
//          FActiveImage := LoopPoint;
          ...
          // Loop to animate
          if (FImage.Images.Count>0)
          and (ActiveImage < FImage.Images.Count) and (ActiveImage >-1) and not(//Terminated or
                                                                              DoRestart) then
          begin
            // Ignore empty images
...
//            begin
              ...
              and (FImage.Images[ActiveImage].GraphicControlExtension.Delay > 0) then
              begin
                ...
                if (Delay > GIFMaximumDelay) //and not(goAsync in DrawOptions)
                then
                begin
                  Delay := GIFMaximumDelay; // Max 10 seconds
                end;

                Self.FEventTimer.Interval:=Delay*GIFDelayExp;
              end
              else
              begin
                Self.FEventTimer.Interval:=Self.FAnimationSpeed*GIFDelayExp;
              end;
//            end;
...
//            end;
        ...
        if FActiveImage>=Self.FImage.Images.Count then
        begin
          FActiveImage := 0;
          CanInc:=False;
        end;
...
//    end;
  ...
  if Image<>nil then
  begin
    Image.Painters.Remove(self,False);
  end;
  ...
  if (BackupBuffer <> nil) then
  begin
    FreeAndNil(BackupBuffer);
  end;
  if (FrameBuffer <> nil) then
  begin
    FreeAndNil(FrameBuffer);
  end;
  if (BackupBufferCanvas <> nil) then
  begin
    FreeAndNil(BackupBufferCanvas);
  end;
  if (FrameBufferCanvas <> nil) then
  begin
    FreeAndNil(FrameBufferCanvas);
  end;
...
//    Background.Free;
  ...
  if Self.FEventTimer<>nil then
  begin
    Self.FEventTimer.Enabled:=False;
  end;
  // Delete event handle
//  if (FEventHandle <> 0) then
...
procedure TGIFPainter.DoPaint(FCanvas:TObject;ADrawPictureParam:TDrawPictureParam;FRect:TRectF);
begin
...
//      BackupBuffer.Width := FRect.Right-FRect.Left;
      ...
      FrameBuffer.Height := Ceil(FRect.Bottom-FRect.Top);
      FrameBuffer.Width := Ceil(FRect.Right-FRect.Left);
//    end;

    if (ActiveImage>=0) and (ActiveImage<FImage.Images.Count) then
    begin
      {$IFDEF FMX}
      FrameBuffer.Canvas.BeginScene();
      try
        FrameBuffer.Canvas.DrawBitmap(FImage.Images[ActiveImage].Bitmap,
            RectF(0,0,FImage.Images[ActiveImage].Width,FImage.Images[ActiveImage].Height),
            RectF(FImage.Images[ActiveImage].ScaleRect(FRect).Left,
                  FImage.Images[ActiveImage].ScaleRect(FRect).Top,
                  FImage.Images[ActiveImage].ScaleRect(FRect).Right,
                  FImage.Images[ActiveImage].ScaleRect(FRect).Bottom),
            1);
      finally
        FrameBuffer.Canvas.EndScene;
      end;
      TDrawCanvas(FCanvas).Canvas.DrawBitmap(FrameBuffer,
                        RectF(0,0,FrameBuffer.Width,FrameBuffer.Height),
                        RectF(FRect.Left,FRect.Top,FRect.Right,FRect.Bottom),
                        1);

      {$ENDIF}
      {$IFDEF VCL}
  //    TDrawCanvas(FrameBufferCanvas).DrawSkinPicture(ADrawPictureParam,
  //                                  TSkinPicture(FImage.Images[ActiveImage].SkinPicture),
  //                                  FImage.Images[ActiveImage].ScaleRect(FRect));
      FrameBuffer.Canvas.Draw(
                  ControlSize(FImage.Images[ActiveImage].ScaleRect(FRect).Left),
                  ControlSize(FImage.Images[ActiveImage].ScaleRect(FRect).Top),
                  FImage.Images[ActiveImage].Bitmap);
      TDrawCanvas(FCanvas).Canvas.StretchDraw(Rect(ControlSize(FRect.Left),ControlSize(FRect.Top),ControlSize(FRect.Right),ControlSize(FRect.Bottom)),FrameBuffer);
      {$ENDIF}
    end;
...
//  end;
//  FImage.Images[ActiveImage](FCanvas, FRect, (goTransparent in FDrawOptions),
//    (goTile in FDrawOptions));
  FStarted := True;
end;

// Buffered paint - Executed in main thread
procedure TGIFPainter.DoPaintFrame(FCanvas:TObject;ADrawPictureParam:TDrawPictureParam;FRect:TRectF);
var
  DrawDestination	: TBitmap;
  DrawDestinationCanvas	: TCanvas;
  DrawRect		: TRectF;
  DoStep2		,
  DoStep3		,
  DoStep5		,
...
//      end
//      else
//    BackupBuffer.Canvas.FillRect(RectF(FRect.Left,FRect.Top,FRect.Right,FRect.Bottom),0,0,[],1);
...
//  if ((Disposals * [dmPrevious, dmBackground]) <> []) then
  begin
    BackupBuffer.Height := Ceil(FRect.Bottom-FRect.Top);
    BackupBuffer.Width := Ceil(FRect.Right-FRect.Left);
...
//  if FActiveImage=0 then
//  begin
//    FCanvas.Clear(0);
//    FrameBuffer.Clear(0);
//    BackupBuffer.Clear(0);
//  end;
...
+------------+------------------+---------------------+------------------------+
  ...
  case (Disposal) of
    dmNone, dmNoDisposal:
      begin
        DrawDestination := BackupBuffer;
        DrawDestinationCanvas:=DrawDestination.Canvas;
        DoStep5 := True;
      end;
    dmBackground:
      begin
      ...
      begin
        case (OldDisposal) of
          ...
          begin
            DrawDestination := nil;
            //{$IFDEF FMX}
            DrawDestinationCanvas:=TDrawCanvas(FCanvas).Canvas;
            //{$ENDIF}
          ...
          begin
            DrawDestination := FrameBuffer;
            DrawDestinationCanvas:=FrameBuffer.Canvas;
            DoStep3 := True;
            DoStep5 := True;
          end;
        end;
...
//    ClearBackup;
  ...
  begin
    ...
    FrameBuffer.Canvas.BeginScene();
    try
      FrameBuffer.Canvas.DrawBitmap(BackupBuffer,
                  RectF(0,0,BackupBuffer.Width,BackupBuffer.Height),
                  RectF(0,0,FrameBuffer.Width,FrameBuffer.Height),
                  1
                  );
    finally
    ...
    {$ENDIF}
    {$IFDEF VCL}
      FrameBuffer.Canvas.Draw(0,0,BackupBuffer);
    {$ENDIF}
  end;

  // Step 4: Draw frame
  if (DrawDestinationCanvas <> nil) then
  begin
    {$IFDEF FMX}
    DrawDestinationCanvas.BeginScene();
    try

      if IsClearDrawDestinationCanvas then DrawDestinationCanvas.Clear(0);


      DrawDestinationCanvas.DrawBitmap(
            FImage.Images[ActiveImage].Bitmap,
            RectF(0,0,FImage.Images[ActiveImage].Width,FImage.Images[ActiveImage].Height),
            RectF(FImage.Images[ActiveImage].ScaleRect(FRect).Left,
                  FImage.Images[ActiveImage].ScaleRect(FRect).Top,
      ...
      DrawDestinationCanvas.EndScene;
    end;
    {$ENDIF}
    {$IFDEF VCL}
    DrawDestinationCanvas.Draw(
                                ControlSize(FImage.Images[ActiveImage].ScaleRect(FRect).Left),
                                ControlSize(FImage.Images[ActiveImage].ScaleRect(FRect).Top),
                                FImage.Images[ActiveImage].Bitmap);
    {$ENDIF}
...
//    FCanvas.CopyMode := cmSrcCopy;
    ...
    {$IFDEF FMX}
    TDrawCanvas(FCanvas).Canvas.DrawBitmap(DrawDestination,
                        RectF(0,0,DrawDestination.Width,DrawDestination.Height),
                        RectF(FRect.Left,FRect.Top,FRect.Right,FRect.Bottom),
                        1);
    {$ENDIF}
    {$IFDEF VCL}
    TDrawCanvas(FCanvas).Canvas.StretchDraw(Rect(ControlSize(FRect.Left),ControlSize(FRect.Top),ControlSize(FRect.Right),ControlSize(FRect.Bottom)),DrawDestination);
    {$ENDIF}

    end;
//    FCanvas.Copy(FRect, DrawDestination, DrawRect);
  end;
...
////      while (DoRestart) do and not(Terminated) do
//      begin
//        FActiveImage := 0;
...
//
//          // Loop to animate
//          while (ActiveImage < FImage.Images.Count) and not(//Terminated or
//                                DoRestart) do
//          begin
//            // Ignore empty images
//            if (FImage.Images[ActiveImage].Empty) then
...
//
////////////////////////////////////////////////////////////////////////////////
// Used by TSkinGIFImage to optimize local color maps to a single global color map.
...
//  public
//    constructor Create(AImage: TSkinGIFImage);
//    destructor Destroy; override;
//    procedure Optimize;
//  end;
//
...
//          Pixel^ := Byte(NewTransparentColorIndex)
//        else
//          // ... all other pixels to their new color index
//          Pixel^ := Byte(ReverseMap[ord(Pixel^)]);
//        dec(size);
//        inc(Pixel);
//      end;
//    end;
//end;
//
//constructor TColorMapOptimizer.Create(AImage: TSkinGIFImage);
//begin
//  inherited Create;
//  FImage := AImage;
...
//
////////////////////////////////////////////////////////////////////////////////
constructor TSkinGIFImage.Create;//(AOwner:TComponent);
begin
  inherited Create;//(AOwner);

//  FGIFFile:=TMemoryStream.Create;
  {$IFDEF FAST_AS_HELL}
  ...
  FPainters := TGIFPainterList.Create;
//  FGlobalPalette := 0;
  // Load defaults
...
destructor TSkinGIFImage.Destroy;
var
  i			: integer;
begin
  PaintStop;
...
//    try
      for i := FPainters.Count-1 downto 0 do
        TGIFPainter(FPainters[i]).FImage := nil;
//    finally
...
procedure TSkinGIFImage.NewImage;
begin
  Clear;
end;

function TSkinGIFImage.GetVersion: TGIFVersion;
var
  v			: TGIFVersion;
  i			: integer;
begin
...
function TSkinGIFImage.GetColorResolution: integer;
var
  i			: integer;
begin
...
function TSkinGIFImage.GetBitsPerPixel: integer;
var
  i			: integer;
begin
...
function TSkinGIFImage.GetBackgroundColorIndex: BYTE;
begin
  Result := FHeader.BackgroundColorIndex;
end;

procedure TSkinGIFImage.SetBackgroundColorIndex(const Value: BYTE);
begin
  FHeader.BackgroundColorIndex := Value;
end;

function TSkinGIFImage.GetBackgroundColor: TColor;
begin
  Result := FHeader.BackgroundColor;
end;

procedure TSkinGIFImage.SetBackgroundColor(const Value: TColor);
begin
  FHeader.BackgroundColor := Value;
end;

function TSkinGIFImage.GetAspectRatio: BYTE;
begin
  Result := FHeader.AspectRatio;
end;

procedure TSkinGIFImage.SetAspectRatio(const Value: BYTE);
begin
  FHeader.AspectRatio := Value;
end;

procedure TSkinGIFImage.SetDrawOptions(Value: TGIFDrawOptions);
begin
...
procedure TSkinGIFImage.SetAnimationSpeed(Value: integer);
var
  I: Integer;
begin
  if (Value < 0) then
  ...
  begin
    ...
    for I := Self.FPainters.Count-1 downto 0 do
    begin
//      TGIFPainter(Self.FPainters[I]).GIFDelayExp:=GIFDelayExp;
      TGIFPainter(Self.FPainters[I]).AnimationSpeed:=FAnimationSpeed;
    end;
...
procedure TSkinGIFImage.SetGIFDelayExp(Value: integer);
var
  I: Integer;
begin
  if (Value <> FGIFDelayExp) then
  begin
    FGIFDelayExp := Value;
    // Use the FPainters threadlist to protect FDrawPainter from being modified
    // by the thread while we mess with it
    ...
    for I := Self.FPainters.Count-1 downto 0 do
    begin
      TGIFPainter(Self.FPainters[I]).GIFDelayExp:=FGIFDelayExp;
//      TGIFPainter(Self.FPainters[I]).AnimationSpeed:=FAnimationSpeed;
    end;
...
//procedure TSkinGIFImage.SetReductionBits(Value: integer);
//begin
...
//procedure TSkinGIFImage.OptimizeColorMap;
//var
...
//procedure TSkinGIFImage.Optimize(Options: TGIFOptimizeOptions;
//  ColorReduction: TColorReduction; DitherMode: TDitherMode;
//  ReductionBits: integer);
//var
//  i			,
...
procedure TSkinGIFImage.SaveToStream(Stream: TStream);
var
  n			: Integer;
  AGIFTrailer:TGIFTrailer;
begin
//  Progress(Self, psStarting, 0, False, Rect(0,0,0,0), sProgressSaving);
  try
    ...
    // Write trailer
    AGIFTrailer:=TGIFTrailer.Create(self);
//    with  do
//      try
        AGIFTrailer.SaveToStream(Stream);
        FreeAndNil(AGIFTrailer);
//      finally
//        Free;
//      end;
  finally
...
procedure TSkinGIFImage.LoadFromFile(FileName: String);
var
  AFileStream:TFileStream;
begin
  AFileStream:=TFileStream.Create(FileName,fmOpenRead or fmShareDenyNone);
  try
    Self.LoadFromStream(AFileStream);
  finally
    FreeAndNil(AFileStream);
  end;
end;

procedure TSkinGIFImage.LoadFromStream(Stream: TStream);
var
  n			: Integer;
  Position		: integer;
  AGIFTrailer:TGIFTrailer;
...
//  Progress(Self, psStarting, 0, False, Rect(0,0,0,0), sProgressLoading);
  try
    ...
    if Not FHeader.Check(Stream) then Exit;
...
//        try
          AGIFTrailer.LoadFromStream(Stream);
          FreeAndNil(AGIFTrailer);
//        finally
//          Free;
//        end;
...
//      if Stream<>FGIFFile then
//      begin
//        Position := Stream.Position;
//        FGIFFile.LoadFromStream(Stream);
//        Stream.Position:=Position;
//      end
//      else
//      begin
...
//    uBaseLog.OutputDebugString('LoadFromStream succ');
...
function TSkinGIFImage.GetBitmap: TBitmap;
begin
  if FBitmap=nil then// or not(FBitmap.IsEmpty) then
  begin
    Result := FBitmap;
    if (Result <> nil) then
      exit;
...
//        FBitmap.Canvas.DrawBitmap(Images[0].Bitmap,
//                    RectF(0,0,FBitmap.Width,FBitmap.Height),
//                    RectF(0+Images[0].Left,
//                          0+Images[0].Top,
//                          Images[0].Left+Images[0].Bitmap.Width,
//                          Images[0].Top+Images[0].Bitmap.Height),
//                    1
//                    );
//        Images[0].Bitmap(FBitmap.Canvas, FBitmap.Canvas.ClipRect, False, False);
      end;
    end;
  end else
    Result := FBitmap;
...
function TSkinGIFImage.NewBitmap: TBitmap;
begin
  Result := FBitmap;
  if (Result <> nil) then
    exit;
...
procedure TSkinGIFImage.FreeBitmap;
begin
//  if (DrawPainter <> nil) then
//    DrawPainter.Stop;
...
function TSkinGIFImage.Add(Source: TPersistent): integer;
var
  Image			: TGIFSubImage;
begin
  ...
  begin
    ...
    try
      ...
      // TGIFimage does not take ownership of Source after TSkinGIFImage.Add() and
...
function TSkinGIFImage.GetEmpty: Boolean;
begin
  Result := (FImages.Count = 0);
end;

//function TSkinGIFImage.GetGIFFile: TMemoryStream;
//begin
//  Result:=FGIFFile;
//end;

function TSkinGIFImage.GetHeight: Integer;
begin
  Result := FHeader.Height;
end;

function TSkinGIFImage.GetWidth: Integer;
begin
  Result := FHeader.Width;
end;

function TSkinGIFImage.GetIsTransparent: Boolean;
var
  i			: integer;
begin
  Result := False;
...
//function TSkinGIFImage.Equals(Graphic: TGraphic): Boolean;
//begin
//  Result := (Graphic = self);
//end;
//
//function TSkinGIFImage.GetPalette: HPALETTE;
//begin
//  // Check for recursion
//  // (TSkinGIFImage.GetPalette->TGIFSubImage.GetPalette->TSkinGIFImage.GetPalette etc...)
...
//end;
//
//procedure TSkinGIFImage.SetPalette(Value: HPalette);
//var
//  NeedNewBitmap		: boolean;
//begin
...
//
// Obsolete
// procedure TSkinGIFImage.Changed(Sender: TObject);
// begin
//  inherited Changed(Sender);
// end;

procedure TSkinGIFImage.SetHeight(Value: Integer);
var
  i			: integer;
begin
  for i := 0 to Images.Count-1 do
...
procedure TSkinGIFImage.SetWidth(Value: Integer);
var
  i			: integer;
begin
  for i := 0 to Images.Count-1 do
...
//procedure TSkinGIFImage.WriteData(Stream: TStream);
//begin
...
procedure TSkinGIFImage.AssignTo(Dest: TPersistent);
begin
  if (Dest is TBitmap) then
    Dest.Assign(Bitmap)
  else
...
{ TODO 1 -oanme -cImprovement : Better handling of TSkinGIFImage.Assign(Empty TBitmap). }
procedure TSkinGIFImage.Assign(Source: TPersistent);
var
  i			: integer;
  Image			: TGIFSubImage;
begin
  ...
  //
  // TSkinGIFImage import
  //
//  if (Source is TSkinGIFImage) then
//  begin
    Clear;
    // Temporarily copy event handlers to be able to generate progress events
    // during the copy and handle copy errors
//    OnProgress := TSkinGIFImage(Source).OnProgress;
    try
//      FOnWarning := TSkinGIFImage(Source).OnWarning;
//      Progress(Self, psStarting, 0, False, Rect(0,0,0,0), sProgressCopying);
      try
//        Self.FGIFFile.LoadFromStream(TSkinGIFImage(Source).FGIFFile);

        FHeader.Assign(TSkinGIFImage(Source).Header);
//        FThreadPriority := TSkinGIFImage(Source).ThreadPriority;
//        FDrawBackgroundColor := TSkinGIFImage(Source).DrawBackgroundColor;
        FDrawOptions := TSkinGIFImage(Source).DrawOptions;
//        FColorReduction := TSkinGIFImage(Source).ColorReduction;
//        FDitherMode := TSkinGIFImage(Source).DitherMode;

        for i := 0 to TSkinGIFImage(Source).Images.Count-1 do
        begin
          Image := TGIFSubImage.Create(self);
          Image.Assign(TSkinGIFImage(Source).Images[i]);
          Add(Image);
//          Progress(Self, psRunning, MulDiv((i+1), 100, TSkinGIFImage(Source).Images.Count),
...
//procedure TSkinGIFImage.LoadFromClipboardFormat(AFormat: Word; AData: THandle;
//  APalette: HPALETTE);
...
//{$ENDIF}
//
//procedure TSkinGIFImage.SaveToClipboardFormat(var AFormat: Word; var AData: THandle;
...
function TSkinGIFImage.GetColorMap: TGIFColorMap;
begin
  Result := FHeader.ColorMap;
end;

//function TSkinGIFImage.GetDoDither: boolean;
//begin
...
//
//{$IFDEF VER9x}
//procedure TSkinGIFImage.Progress(Sender: TObject; Stage: TProgressStage;
...
//procedure TSkinGIFImage.StopDraw;
////var
////  Msg			: TMsg;
////  ThreadWindow		: HWND;
//begin
...
//procedure TSkinGIFImage.Draw(PaintObject:TObject);//ACanvas: TCanvas; const Rect: TRect);
////var
////  Canvas		: TCanvas;
////  DestRect		: TRect;
////  Msg			: TMsg;
...
//////        (goTile in FDrawOptions));
////      ACanvas.DrawBitmap(FImages[0].Bitmap,
////                        RectF(0,0,FImages[0].Width,FImages[0].Height),
////                        RectF(Rect.Left,Rect.Top,Rect.Right,Rect.Bottom),1);
////      exit;
////    end;
////    else
////    if (FBitmap <> nil) then//and not(goDirectDraw in FDrawOptions) then
////    begin
////      ACanvas.DrawBitmap(Bitmap,
////                        RectF(0,0,FImages[0].Width,FImages[0].Height),
////                        RectF(Rect.Left,Rect.Top,Rect.Right,Rect.Bottom),1);
...
////      if (FDrawPainter <> nil)
////      and (FDrawPainter.Canvas = ACanvas) and
////        EqualRect(FDrawPainter.Rect, Rect)
////        then
////        exit;
//
...
function TSkinGIFImage.Empty: Boolean;
begin
  if Self.FImages.Count>0 then
  begin
//    uBaseLog.OutputDebugString('Self.FImages.Count:'+IntToStr(Self.FImages.Count));
    Result:=Self.FImages[0].Empty;
  end
  else
  begin
    Result:=True;
...
//function TSkinGIFImage.InternalPaint(Painter: PGifPainter;
////  ACanvas: TCanvas;
////  const Rect: TRect;
//  PaintObject:TObject;
//  Options: TGIFDrawOptions): TGIFPainter;
...
//                            goAnimate];
//
//  Result := TGIFPainter.CreateRef(Painter, self,
//  PaintObject,
////  ACanvas, Rect,
//  Options);
//
...
function TSkinGIFImage.Paint(//ACanvas: TCanvas; const Rect: TRect;
                          PaintObject:TObject;
                          Options: TGIFDrawOptions): TGIFPainter;
var
  I: Integer;
begin
  ...
  if PaintObject=nil then Exit;
  
  for I := Self.FPainters.Count-1 downto 0 do
  begin
    if TGIFPainter(Self.FPainters[I]).FPaintObject=PaintObject then
    begin
      Result:=TGIFPainter(Self.FPainters[I]);
      Result.Start;
      Exit;
    end;
  end;

//  Result := InternalPaint(nil,
//  PaintObject,
//  //ACanvas, Rect,
//  Options);
...
//  end;
  ...
  Result := TGIFPainter.Create(Self,
                                  PaintObject,
                                //  ACanvas, Rect,
                                  Options);
...
procedure TSkinGIFImage.PaintStart;
var
  i			: integer;
begin
//  with FPainters.LockList do
//    try
      for i := 0 to FPainters.Count-1 do
        TGIFPainter(FPainters.Items[i]).Start;
//    finally
//      FPainters.UnLockList;
//    end;
...
procedure TSkinGIFImage.PaintStop;
var
//  Ghosts		: integer;
  i			: integer;
//  Msg			: TMsg;
...
procedure TSkinGIFImage.PaintPause;
var
  i			: integer;
begin
//  with FPainters.LockList do
//    try
      for i := 0 to FPainters.Count-1 do
      begin
        TGIFPainter(FPainters[i]).Stop;//Suspend;
      end;
//    finally
//      FPainters.UnLockList;
//    end;
...
procedure TSkinGIFImage.PaintResume;
var
  i			: integer;
begin
...
//    try
      for i := 0 to FPainters.Count-1 do
        TGIFPainter(FPainters[i]).Start;
//    finally
//      FPainters.UnLockList;
//    end;
end;

procedure TSkinGIFImage.PaintRestart;
var
  i			: integer;
begin
//  with FPainters.LockList do
//    try
      for i := 0 to FPainters.Count-1 do
      begin
        TGIFPainter(FPainters[i]).Restart;
      end;
//    finally
//      FPainters.UnLockList;
//    end;
...
procedure TSkinGIFImage.Warning(Sender: TObject; Severity: TGIFSeverity; Message: string);
begin
...
function TGIFPainterList.FindByPaintObject(PaintObject:TObject): TGIFPainter;
var
  I: Integer;
begin
  Result:=nil;
  ...
  begin
    if Items[I].FPaintObject=PaintObject then
    begin
      Result:=Items[I];
      Break;
    end;
...
function TGIFPainterList.GetItem(Index: Integer): TGIFPainter;
begin
  Result:=TGIFPainter(Inherited Items[Index]);
end;
...
//  inherited Create(AOwner);
//  FGIFFile:=TMemoryStream.Create;
//  FGIFImage:=TSkinGIFImage.Create;
//  FGIFImage.OnPaint:=Self.DoPaint;
//end;
//
//destructor TSkinGIFImage.Destroy;
//begin
...
//end;
//
//procedure TSkinGIFImage.DoPaint(Sender: TObject);
//begin
...
//  if Stream=nil then
//  begin
////    uBaseLog.OutputDebugString('SetGIFFile:nil');
//    FGIFFile.Size:=0;
//  end
//  else
//  begin
//  //  uBaseLog.OutputDebugString('ReadData');
//    Position:=Stream.Position;
//    FGIFFile.LoadFromStream(Stream);
//    Stream.Position:=Position;
...
//end;
//
//procedure TSkinGIFImage.DefineProperties(Filer: TFiler);
//begin
//  inherited;
...
//
//
//procedure TSkinGIFImage.ReadData(Stream: TStream);
//begin
//  LoadFromStream(Stream);
//end;
//
//procedure TSkinGIFImage.WriteData(Stream: TStream);
//begin
////  uBaseLog.OutputDebugString('WriteData');
//  if FGIFFile.Size>0 then
//  begin
////    uBaseLog.OutputDebugString('has data');
//    FGIFFile.Position:=0;
//    Stream.CopyFrom(Self.FGIFFile,FGIFFile.Size);
//  end
//  else
//  begin
////    uBaseLog.OutputDebugString('no data');
//  end;
//end;

//procedure TSkinGIFImage.SetGIFFile(const Value: TMemoryStream);
//var
//  Position:Integer;
//begin
...
//  else
//  begin
////    uBaseLog.OutputDebugString('SetGIFFile:'+IntToStr(Value.Size));
//    Position:=Value.Position;
//    FGIFFile.LoadFromStream(Value);
//    Value.Position:=Position;
//  end;
//  FGIFImage.LoadFromStream(Value);
//end;

//procedure TSkinGIFImage.SetGIFFileStream(const Value: TStream);
//var
//  Position:Integer;
//begin
//  if Value=nil then
...
initialization
  IsClearDrawDestinationCanvas:=True;
//{$IFDEF REGISTER_TGIFIMAGE}
//  TPicture.RegisterFileFormat('GIF', sGIFImageFile, TSkinGIFImage);
//  CF_GIF := RegisterClipboardFormat(PByte(sGIFImageFile));
//  TPicture.RegisterClipboardFormat(CF_GIF, TSkinGIFImage);
//{$ENDIF}
//  DesktopDC := GetDC(0);
//  try
...
finalization
  ...
  FreeAndNil(AppExtensionList);
//{$IFNDEF VER9x}
//  {$IFDEF REGISTER_TGIFIMAGE}
//    TPicture.UnregisterGraphicClass(TSkinGIFImage);
//  {$ENDIF}
//  {$IFDEF VER100}
//  if (pf8BitBitmap <> nil) then
//    pf8BitBitmap.Free;
